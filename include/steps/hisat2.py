...
class Hisat2(AbstractStep):
    ...
    def __init__(self, pipeline):
        ...
        self.add_option('index', str, optional=False,
                        description="Path to hisat2 index (not containing file "
                        "suffixes).")
        ...
        for run_id in run_ids_connections_files.keys():
            with self.declare_run(run_id) as run:
                ...
                with run.new_exec_group() as exec_group:
                    ...
                    # 3. Map reads using hisat2
                    with exec_group.add_pipeline() as hisat2_pipe:
                        # Assemble hisat2 command
                        hisat2 = [
                            self.get_tool('hisat2'), '-q',
                            '-p', str(self.get_option('cores')),
                        ...
                        if is_paired_end:
                            ...
                            hisat2.extend(['-U', ','.join(fr_temp_fifos)])
                        ...
                        pigz = [self.get_tool('pigz'),
                                '--stdout']
                        hisat2_pipe.add_command(pigz)
                        # Write hisat2 output to file
                        dd = [
                            self.get_tool('dd'),
                            'obs=4M',
                            'of=%s' %
